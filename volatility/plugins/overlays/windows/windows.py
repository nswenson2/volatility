...
class _EPROCESS(obj.CType, ExecutiveObjectMixin):
    ...
    def get_vads(self, vad_filter = None, skip_max_commit = False):
        ...
        for vad in self.VadRoot.traverse():
            ...
            ##print "pml4e: {0} {1}".format(vad.Start >> 39, hex(vad.Length))
            ...
            if skip_max_commit: 
                ...
                elif vad.Length > 0x0ffffffffff:
                    message = "Ignoring very large vad. Start: {1} pml4e: {3} End: {2} Length: {0}".format(hex(vad.Length), hex(vad.Start), hex(vad.End), vad.Start >> 39)
                    if vad.FileObject and vad.FileObject.FileName:
                        message += " File Name: {0}".format(vad.FileObject.FileName)
