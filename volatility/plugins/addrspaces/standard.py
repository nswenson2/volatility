...
INFO_IOCTRL = CTL_CODE(0x8000, 0x818, 0, 3)

VIRTUAL_ADDRESS_IS_VALID = CTL_CODE(0x8000, 0x819, 0, 3)
...
class PmemAddressSpace(addrspace.AbstractRunBasedMemory):
    """ This is a direct addriver AS.

    For this AS to be instantiated, we need

    1) A valid config.LOCATION (starting with \\.\addriver)
    ...
    def __init__(self, base, config, layered = False, *args, **kwargs):
        ...
        self.as_assert(config.LOCATION.startswith("\\\\"), 'Location is not of addriver scheme')
